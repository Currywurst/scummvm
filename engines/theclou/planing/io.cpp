...
ubyte plOpen(U32 objId, ubyte mode, TC_FILE ** fh)
{
	...
	snprintf(name2, sizeof(name2), "%s%s", name1, PLANING_PLAN_LIST_EXTENSION);
	...
	if ((pllFh = dskOpen(pllPath, "rb"))) {
		...
		snprintf(name2, sizeof(name2), "MODE_%d", mode);
		...
		snprintf(name2, sizeof(name2), "EXPAND_MODE_%d", mode);
		...
		if (ChoiceOk(i, GET_OUT, PlanList)) {
		    ...
		    if ((data = (struct IOData *)GetNthNode(PlanList, i)))
			i = data->io_Data;
		    ...
		    snprintf(name2, sizeof(name2), "%s%d%s", name1, i + 1,
