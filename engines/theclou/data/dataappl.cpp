...
{				/* ptr auf 4 U32s */
    ...
    if (i > 0) team /= i; /* V609: guard against division by zero when team is empty */
			     ...
			     S16 livY)
{
    ...
    for (node = (NODE *)LIST_HEAD(roomsList); NODE_SUCC(node) && detected == 0;
	 node = NODE_SUCC(node)) {
...
bool tcAlarmByPowerLoss(U32 powerId)
{
    ...
    for (n = (NODE *)LIST_HEAD(friendlyList); NODE_SUCC(n);
	 n = NODE_SUCC(n)) {
		   ...
		   U16 height, U32 guardId, U8 livId, U32 areaId)
{
    ...
    snprintf(name, sizeof(name), "Police_%d", livId);
