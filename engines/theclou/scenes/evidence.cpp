...
static ubyte tcCarFound(Car car, U32 time)
{
    ...
    if (car != (Car)dbGetObject(Car_Jaguar_XK_1950)) {
...
static U32 tcATraitor(U32 traitorId)
{
    ...
    snprintf(line, sizeof(line), NODE_NAME(LIST_HEAD(bubble)), name);
    ...
    (void)Bubble(newList, 0, 0L, 0L); /* V1071: display-only, result intentionally discarded */
...
U32 tcStartEvidence(void)
{
	...
	if (p[i] != 0) {
	    ...
	    if (!caught)
		if (p[i] == (Person)dbGetObject(Person_Robert_Bull))
		    caught |= tcATraitor(Person_Robert_Bull);

	    if (!caught)
		if (p[i] == (Person)dbGetObject(Person_Marc_Smith))
		    caught |= tcATraitor(Person_Marc_Smith);

            if (setup.Profidisk) {
	        if (!caught)
		    if (p[i] == (Person)dbGetObject(Person_Phil_Ciggy))
...
U32 tcPersonWanted(U32 persId)
{
    ...
    snprintf(line, sizeof(line), "%s %s.", NODE_NAME(GetNthNode(bubble, 3)), name);
...
U32 tcPersonQuestioning(Person person)
{
    ...
    if (person != (Person)dbGetObject(Person_Matt_Stuvysunt)) {
