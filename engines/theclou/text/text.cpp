...
void txtLoad(U32 textId)
{
	...
	if (!txt->txt_Handle) {
	    ...
	    size_t length;
            ...
            snprintf(txtFile, sizeof(txtFile), "%s%c%s",
                NODE_NAME(txt),
                txtBase->tc_Language,
                TXT_SUFFIX);
...
LIST *txtGoKeyAndInsert(U32 textId, const char *key, ...)
{
    ...
    for (node = (NODE *)LIST_HEAD(originList); NODE_SUCC(node); node = (NODE *)NODE_SUCC(node)) {
	U8 i;
	...
	for (i = 2; i < strlen(originLine); i++) {
	    if (originLine[i - 2] == '%') {
		snprintf(txtLine, sizeof(txtLine), originLine, va_arg(argument, U32));
...
void txtPutCharacter(LIST * list, uword pos, U8 c)
{
    ...
    for (node = (NODE *)LIST_HEAD(list); NODE_SUCC(node); node = (NODE *)NODE_SUCC(node))
