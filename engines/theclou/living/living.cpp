...
#include "living/living_internal.h"
...
{
    gSpriteControl = (struct SpriteControl *)TCAllocMem(sizeof(*gSpriteControl), 0);
...
void livDone(void)
{
	...
	if (gSpriteControl->p_Livings) {
	    ...
	    gSpriteControl->p_Livings = nullptr;
	}
	...
	if (gSpriteControl->p_Template) {
	    ...
	    gSpriteControl->p_Template = nullptr;
	}

	TCFreeMem(gSpriteControl, sizeof(*gSpriteControl));

	gSpriteControl = nullptr;
    }
}
...
static void livLoadLivings(void)
{
    ...
    for (i = 0; i < cnt; i++) {
	char name[TXT_KEY_LENGTH], tmpl[TXT_KEY_LENGTH];

	line = NODE_NAME(GetNthNode(l, i));

	strcpy(name, txtGetKey(1, line));
	strcpy(tmpl, txtGetKey(2, line));
